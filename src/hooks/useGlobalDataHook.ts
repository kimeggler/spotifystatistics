...
const useGlobalDataHook = <T>(
  ...
  console.log(`[useGlobalDataHook] Current state - isLoading: ${isLoading}, data: ${data ? 'exists' : 'null'}, hasError: ${hasError}`);
  ...
  useEffect(() => {
    ...
    console.log(`[useGlobalDataHook] Request string changed: ${requestString !== lastRequestStringRef.current}`);
    ...
    const fetchData = async () => {
      if (!mountedRef.current) {
        console.log(`[useGlobalDataHook] Component unmounted, skipping request for: ${loadingMessage}`);
        return;
      }
      ...
      try {
        ...
        if (!mountedRef.current) {
          console.log(`[useGlobalDataHook] Component unmounted during error handling: ${loadingMessage}`);
          return;
        }
        ...
        if (mountedRef.current) {
          ...
          if (globalLoadingCounter <= 0) {
            ...
            console.log(`[useGlobalDataHook] Keeping global loading TRUE - ${globalLoadingCounter} requests still active`);
          }
        }
      }
    };
